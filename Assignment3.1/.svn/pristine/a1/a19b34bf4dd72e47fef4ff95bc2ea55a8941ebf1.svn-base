...
from src.view.server import db


class TestComment(unittest.TestCase):
    @classmethod
    def setUp(cls):
        cls.comment_obj = Comment(db)
    ...
    def test_increment_message_id(self):
        self.assertEqual(self.comment_obj.increment_message_id('a'), 0)
        self.assertEqual(self.comment_obj.increment_message_id('a'), 1)
        self.assertEqual(self.comment_obj.increment_message_id('a'), 2)
        result = Message_count.query.first()
        self.assertEqual(result.num_comments, 2)

    def test_get_last_child(self):
        message_1 = Message(0, 'a', 'comment 0', -1)
        message_2 = Message(1, 'a', 'comment 1', message_1.message_id)
        message_3 = Message(2, 'a', 'comment 2', message_2.message_id)
        message_4 = Message(0, 'b', 'comment 0', -1)
    ...
    def test_get_last_subthread_child_id(self):
        ...
        message_2 = Message(1, 'a', 'comment 1', message_1.message_id)
        message_3 = Message(2, 'a', 'comment 2', message_2.message_id)
        message_4 = Message(3, 'a', 'comment 0a', message_1.message_id)
        message_5 = Message(4, 'a', 'comment 0b', message_4.message_id)
        message_6 = Message(5, 'a', 'comment 1a', message_2.message_id)
        message_1.child = message_2.message_id
    ...
    def test_get_message(self):
        ...
        self.assertEqual(result_json,
                         {'message': 'head node', 'votes':0, 'id': head_message.message_id, 'subthread': {
                             'message': 'message a0.1 sub', 'votes':0, 'id': sub_message_1.message_id, 'subthread': {
                                 'message': 'message a0.1 sub_sub thread', 'votes':0, 'id': sub_sub_message_1.message_id,
                                 'subthread': None, 'child_message': None
                             },
                             'child_message': None
                         },
                          'child_message': {'message': 'message a1', 'votes':0, 'id': message_1.message_id, 'subthread': {
                              'message': 'message a1.1 sub', 'votes':0, 'id': sub_message_2.message_id, 'subthread': None,
                              'child_message': None
                          },
                                            'child_message': {
                                                'message': 'message a2', 'votes':0, 'id': message_2.message_id, 'subthread': None,
                                                'child_message': None
                                            }}})

    def test_get_sort_by_votes(self):
        head_message = self.comment_obj.post_message('a', 'head node', -1)
        message_1 = self.comment_obj.post_message('a', 'message a1', head_message.message_id)
        message_2 = self.comment_obj.post_message('a', 'message a2', head_message.message_id)
        sub_message_1 = self.comment_obj.post_message('a', 'message a0.1 sub', head_message.message_id,
        ...
        sub_sub_message_1 = self.comment_obj.post_message('a', 'message a0.1 sub_sub thread', sub_message_1.message_id,
                                                          True)
        self.assertEqual(self.comment_obj.increment_message_votes('a', head_message.message_id), 1)
        self.assertEqual(self.comment_obj.increment_message_votes('a', head_message.message_id), 2)
        self.assertEqual(self.comment_obj.increment_message_votes('a', head_message.message_id), 3)
        self.assertEqual(self.comment_obj.increment_message_votes('a', head_message.message_id), 4)
        self.assertEqual(self.comment_obj.increment_message_votes('a', message_1.message_id), 1)
        self.assertEqual(self.comment_obj.increment_message_votes('a', message_1.message_id), 2)
        result_json = self.comment_obj.get_message('a', True)
        self.assertEqual(result_json,
                         {'message': 'head node', 'votes': 4, 'id': head_message.message_id, 'subthread': {
                             'message': 'message a0.1 sub', 'votes': 0, 'id': sub_message_1.message_id, 'subthread': {
                                 'message': 'message a0.1 sub_sub thread', 'votes': 0,
                                 'id': sub_sub_message_1.message_id,
                                 'subthread': None, 'child_message': None
                             },
                             'child_message': None
                         },
                          'child_message': {'message': 'message a1', 'votes': 2, 'id': message_1.message_id,
                                            'subthread': {
                                                'message': 'message a1.1 sub', 'votes': 0,
                                                'id': sub_message_2.message_id, 'subthread': None,
                                                'child_message': None
                                            },
                                            'child_message': {
                                                'message': 'message a2', 'votes': 0, 'id': message_2.message_id,
                                                'subthread': None,
                                                'child_message': None
                                            }}})
        self.assertEqual(self.comment_obj.increment_message_votes('a', message_2.message_id), 1)
        self.assertEqual(self.comment_obj.increment_message_votes('a', message_2.message_id), 2)
        self.assertEqual(self.comment_obj.increment_message_votes('a', message_2.message_id), 3)
        self.assertEqual(self.comment_obj.get_message('a', True),
                         {'message': 'head node', 'votes': 4, 'id': head_message.message_id, 'subthread': {
                             'message': 'message a0.1 sub', 'votes': 0, 'id': sub_message_1.message_id, 'subthread': {
                                 'message': 'message a0.1 sub_sub thread', 'votes': 0,
                                 'id': sub_sub_message_1.message_id,
                          ...
                          'child_message': {'message': 'message a2', 'votes': 3, 'id': message_2.message_id,
                                            'subthread':None,
                                            'child_message': {
                                                'message': 'message a1', 'votes': 2, 'id': message_1.message_id,
                                                'subthread':  {
                                                    'message': 'message a1.1 sub', 'votes': 0,
                                                    'id': sub_message_2.message_id, 'subthread': None,
                                                    'child_message': None
