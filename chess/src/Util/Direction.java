...
public class Direction {
    ...
    public Direction(Position initial, Position end, int distance) throws Exception {
        int[] initial_array = initial.getPositionArray().clone();
        int[] end_array = end.getPositionArray().clone();
    ...
    public boolean canMoveInDirection(Position currentPosition, Position targetPosition, Board board) throws Exception {
        ...
        int[] currentPositionArr = currentPosition.getPositionArray().clone();
        int[] targetPositionArr = targetPosition.getPositionArray().clone();
        for (int idx = 0; idx < directionVectorArr.length; idx++) {
            directionVectorArr[idx] = targetPositionArr[idx] - currentPositionArr[idx];
        }
        if (this.isEqual(directionVector)) {
            ...
            if (this.distance == -1) {
                ...
                while (!board.isOutOfBound(copyPosition)) {
                    ...
                    if (currentPositionPiece == null) {
                        continue;
                    } else if (currentPositionPiece.getUserId() == currentPiece.getUserId()) {
                ...
                for (int dist = 0; dist < this.distance; dist++) {
                    copyPosition.moveByDirection(this, 1);
                    Piece pathPiece = board.getPiece(copyPosition);
                    if (pathPiece != null && currentPiece.getUserId() == pathPiece.getUserId()) {
                        return false;
                    } else if (copyPosition.isEqual(targetPosition)) {
                        return true;
                    }
     ...
     */
    public boolean isEqual(Direction otherDirection) {
        boolean isEqual = true;
        int[] otherDirectionVector = otherDirection.getDirection();
        int[] copyDirVector = this.dirVector.clone();
        for (int idx = 0; idx < this.dirVector.length; idx++) {
            int gcd = this.gcd(this.dirVector[idx], otherDirectionVector[idx]);
            boolean canReach = (this.distance == -1 || (this.distance * Math.abs(this.dirVector[idx]) >= gcd && gcd != 0) || gcd == 0);
            if (gcd != 0 && this.distance == -1) {
                otherDirectionVector[idx] = otherDirectionVector[idx] / gcd;
            }
            isEqual = isEqual && (this.dirVector[idx] == otherDirectionVector[idx] && canReach);
        }
        return isEqual;
    }
    ...
    private void simplify() {
        ...
        if (gcd == 0) {
            if (this.distance == -1) {
                for (int idx = 0; idx < this.dirVector.length; idx++) {
                    if (this.dirVector[idx] != 0) {
                        this.dirVector[idx] = this.dirVector[idx] / Math.abs(this.dirVector[idx]);
                    }
                }
            }
            return;
